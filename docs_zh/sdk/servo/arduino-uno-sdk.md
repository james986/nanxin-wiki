# 总线伺服舵机 SDK 使用手册（Arduino Uno）

<!--一以下是SDK页面用的表格宏（适用协议/版本/GitHub链接/单个下载文件-->
{{ github_download_table("", "v1.2026.0214", "https://github.com/servodevelop/servo-uart-rs485-sdk", "./data/arduino-sdk.zip", "28px") }}

<div class="fs-body-code" markdown="1">
...
{{ fig_indent_adjustable("../images/arduino-sdk/image01.png","image01", "100%", "3.5em") }}

{{ fig_indent_adjustable("../images/arduino-sdk/image02.png","image02", "100%", "3.5em") }}
...
{{ fig_indent_adjustable("../images/arduino-sdk/Arduino%E8%BD%AF%E4%B8%B2%E5%8F%A3USB%E8%BD%ACTTL%E6%A8%A1%E5%9D%97%E5%AE%9E%E7%89%A9%E5%9B%BE.jpg","Arduino软串口USB转TTL模块实物图", "100%", "3.5em") }}

**·** 将`USB`转`TTL`模块插入到电脑的`USB`口上

**·** 打开串口调试助手，选择`USB`转`TTL`模块的端口号，配置如下图所示：

{{ fig_indent_adjustable("../images/arduino-sdk/%E4%B8%B2%E5%8F%A3%E8%B0%83%E8%AF%95%E5%8A%A9%E6%89%8B%E7%9A%84%E9%85%8D%E7%BD%AE.png","串口调试助手的配置", "100%", "3.5em") }}
...
{{ fig_indent_adjustable("../images/arduino-sdk/test_software_serial.png","test_software_serial", "100%", "3.5em") }}
...
void loop(){
   ...
   uservo.queryRawAngleMTurn();      //读取角度
    ...
    // 等待1s
    delay(1000);
    uservo.StopOnControlUnloading(); //停止舵机（清除圈数需要在舵机停止状态）
    delay(10); 
    uservo.ResetMultiTurnAngle();   //清除当前圈数
    delay(10);
    uservo.queryRawAngleMTurn();    //读取角度
...
```c++
void FSUS_Servo::BeginAsync();	//异步写入异步指令

void FSUS_Servo::EndAsync(uint8_t cancel);  // 异步执行指令
```
...
void loop(){
    ...
    */
    delay(1000);
    uservo.EndAsync(0);         //异步执行，执行存入的指令
    delay(1000);
...
```c++
void StopOnControlMode(uint8_t method, uint16_t power);// 停止指令
    
void StopOnControlUnloading();// 停止指令-卸力（失锁）
   
void StopOnControlKeep(uint16_t power); // 停止指令-锁力
    
void StopOnControlDammping(uint16_t power);// 停止指令-阻尼
```
